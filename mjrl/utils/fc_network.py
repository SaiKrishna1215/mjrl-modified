...
class BidirectionalLSTMNetwork(nn.Module):
    def __init__(self, obs_dim, act_dim,
                 ...
                 out_shift=None,
                 out_scale=None,
                 log_dir='runs/bilstm_activations'):
        ...
        self.fc = nn.Linear(lstm_hidden_size * 2, mlp_hidden_size)
    ...
    def initialize_hidden_state(self, batch_size, device):
        return (
            torch.zeros(2, batch_size, self.lstm.hidden_size).to(device),  # h0
            torch.zeros(2, batch_size, self.lstm.hidden_size).to(device)   # c0
        )

    def forward(self, x, hidden_state=None):
        if self.hidden_state is None or x.size(0) != self.batch_size:
            self.batch_size = x.size(0)
            self.hidden_state = self.initialize_hidden_state(self.batch_size, x.device)

        x = (x - self.in_shift.to(x.device)) / (self.in_scale.to(x.device) + 1e-8)
        if x.dim() == 2:
            x = x.unsqueeze(1)  # (B, T=1, obs_dim)

        lstm_out, self.hidden_state = self.lstm(x, self.hidden_state)  # (B, T, 2H)
        ...
        if self.use_attention:
            ...
            context = lstm_out[:, -1, :]  # last timestep output from both directions

        out = self.nonlinearity(self.fc(context))
        self.activations.append(out)
        out = self.output_layer(out)
        out = out * self.out_scale.to(x.device) + self.out_shift.to(x.device)
